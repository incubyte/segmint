...
import { getApiKeys, createApiKey, deleteApiKey } from "@/services/settingsService";
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Manage API keys for accessing Segmint services programmatically</p>
      </div>

      <Alert>
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>
          API keys provide full access to your Segmint account. Keep them secure and never share them in public repositories or client-side code.
        </AlertDescription>
      </Alert>
        ...
        <CardContent>
          ...
          {apiKeys.length === 0 ? (
            ...
            <div className="border rounded-md divide-y">
              {apiKeys.map((apiKey) => (
                <div key={apiKey.id} className="flex items-center justify-between p-4">
                  <div>
                    <div className="font-medium">{apiKey.name}</div>
                    <div className="text-sm text-muted-foreground">
                      <span className="font-mono">{apiKey.key.slice(0, 10)}•••••••••</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <AlertDialog open={isDeleteDialogOpen && keyToDelete === apiKey.id} onOpenChange={(open) => {
